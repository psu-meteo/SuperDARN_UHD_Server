...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    // for DEBUG
    clock_t debug_start, debug_stop;
    double elapsed_secs;     
        ...
        while(true) {
            ...
            switch(command) {
                ...
                //        DEBUG_PRINT("USRP_SETUP received %zu pulse offset\n", pulse_sample_idx_offsets[i]);
                    ...
                    // DEBUG timing
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP rx par : %f\n", elapsed_secs);
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP resize : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP reset rxtx rates : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP reset rx tx freq : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP minor init stuff : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP resize tx_samples var : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP zero padding tx : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                    ...
                    debug_stop = clock();
                    elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                    DEBUG_PRINT("#bench SETUP finish : %f\n", elapsed_secs);
                    debug_start = debug_stop;
                ...
                case TRIGGER_PULSE: {
                    ...
                    if (state_vec[swing] != ST_READY) {
                        ...
                        DEBUG_PRINT("TRIGGER_PULSE busy in state_vec[swing] %d, returning\n", state_vec[swing]);
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER init        : %f\n", elapsed_secs);
                         debug_start = debug_stop;
                        ...
                        DEBUG_PRINT("TRIGGER_PULSE semaphore locked\n");
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER acquire sem : %f\n", elapsed_secs);
                         debug_start = debug_stop;
                        ...
                        double tr_to_pulse_delay = sock_get_float64(driverconn);
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER init&rx var : %f\n", elapsed_secs);
                         debug_start = debug_stop;
                        ...
                        rx_start_time = offset_time_spec(rx_start_time, pulse_sample_idx_offsets[0]/txrate); 
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER calc offsets: %f\n", elapsed_secs);
                         debug_start = debug_stop;
                        ...
                        uhd_threads.create_thread(boost::bind(usrp_rx_worker, usrp, rx_stream, &rx_data_buffer, nSamples_rx, rx_start_time, &rx_worker_status));
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER start rx wor: %f\n", elapsed_secs);
                         debug_start = debug_stop;
                        ...
                        if (tx_worker_active) { 
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER start tx wor: %f\n", elapsed_secs);
                         debug_start = debug_stop;
                        ...
                        uhd_threads.create_thread(boost::bind(send_timing_for_sequence, usrp, start_time,  pulse_time_offsets, pulseLength, mimic_active, mimic_delay, nSides)); 
                         ...
                         debug_stop = clock();
                         elapsed_secs = double(debug_stop - debug_start) / CLOCKS_PER_SEC;
                         DEBUG_PRINT("#bench TRIGGER start DIO   : %f\n", elapsed_secs);
                         debug_start = debug_stop;
