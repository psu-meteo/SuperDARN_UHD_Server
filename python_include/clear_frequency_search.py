...
def calc_clear_freq_on_raw_samples(raw_samples, sample_meta_data, restricted_frequencies, clear_freq_range, beam_angle, smsep, bb_bandwidth):
    # unpack meta data
    ...
    # freq_delta=freq_vector[1]-freq_vector[0]
    # n_pts_avg=int(bb_bandwidth/freq_delta)
    # fv_len=int(len(freq_vector)/n_pts_avg)
    # freq_vec_search=np.zeros(fv_len)
    # spec_avg=np.zeros(fv_len)
    # for j in range(fv_len):
    #    freq_vec_search[j]=np.sum(freq_vector[j*n_pts_avg:j*n_pts_avg+n_pts_avg])/n_pts_avg
    #    spec_avg[j]=np.sum(spectrum_power[j*n_pts_avg:j*n_pts_avg+n_pts_avg])/n_pts_avg

    # freq_vector=freq_vec_search
    # spectrum_power=spec_avg
