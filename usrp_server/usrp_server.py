...
RESTRICT_FILE = \
    '/home/radar/repos/SuperDARN_MSI_ROS/linux/home/radar/ros.3.6/tables/superdarn/site/site.kod/restrict.dat.inst'
nSwings = 2
...
class statusUpdater():
    ...
    def create_status_information(self):
        ...
        status += "USRPs: {} active, {} inactive\n".format(len(self.RHM.usrpManager.addressList_active),
                                                           len(self.RHM.usrpManager.addressList_inactive))
...
class usrpSockManager():
    def __init__(self, RHM):
        ...
        # open each
        self.hostnameList_active = []
        # TODO: make connecting the usrps its own function
    ...
    def remove_sock(self, sock_to_remove):
        ...
        self.logger.error(
            "Removing usrp {} ({}:{}). ".format(self.hostnameList_active[iSock], self.addressList_active[iSock][0],
                                                self.addressList_active[iSock][1]))
        ...
        if CONNECTION_ERROR in client_return:
            ...
            for iSock, singleReturn in enumerate(client_return):
                if singleReturn == CONNECTION_ERROR:
                    self.logger.error("Connection lost to usrp {}:{}. Removing it from sock list. ".format(
                        self.addressList_active[iSock - offset][0], self.addressList_active[iSock - offset][1]))
    ...
    def watchdog(self, all_usrps_report_failure):
        if all_usrps_report_failure:
            ...
            if self.errors_in_a_row >= self.error_limit:
                self.logger.error(
                    "All USRPs reported error for GET_DATA {} times in a row. Shutting down usrp_server".format(
...
class usrpMixingFreqManager():
    ...
    def __init__(self, cFreq, bandWidth):
        ...
        self.usrp_bandwidth = bandWidth - USRP_BANDWIDTH_RESTRICTION * 2 / 1000  # in kHz (for control program)
    ...
    def add_new_freq_band(self, channel):
        ...
        if newLower < RHM.hardwareLimit_freqRange[0] or newUpper > RHM.hardwareLimit_freqRange[1]:
            channel.logger.error(
                "Channel bandwidth ({} MHz- {} MHz) is not covered by radar hardware limits ({} MHz - {} MHz)".format(
                    newLower / 1000, newUpper / 1000, RHM.hardwareLimit_freqRange[0] / 1000,
                    RHM.hardwareLimit_freqRange[1] / 1000))
            return False
        ...
        if newLower > (self.current_mixing_freq - self.usrp_bandwidth / 2) and newUpper < (
                self.current_mixing_freq + self.usrp_bandwidth / 2):
            ...
            if (allCh_upper - allCh_lower) > self.usrp_bandwidth:
                ...
                channel.logger.info(
                    "calculated new usrp mixing frequency: {} kHz (old was {} kHz)".format(newMixingFreq,
                                                                                           self.current_mixing_freq))
...
class clearFrequencyRawDataManager():
    ...
    def record_new_data(self):
        ...
        if rec_new_samples:
            ...
            self.rawData, self.antennaList = record_clrfreq_raw_samples(self.usrpManager.get_all_main_antenna_socks(),
                                                                        self.number_of_samples, self.center_freq,
                                                                        self.sampling_rate)
            self.logger.debug('end record_clrfreq_raw_samples')

            self.metaData['antenna_list'] = self.antennaList
            self.logger.debug(
                "recorded clear samples for clear frequency search, antenna list: {}".format(self.antennaList))
            ...
            self.logger.debug(
                "clearFreqRawData: age of data is {:2.2f} s. No need to record new data...".format(data_age))
    ...
    def get_raw_data(self):
        ...
        if self.rawData is None:
            ...
            print("clearFreqDataManager: provide raw data (age {}), setting raw_rec request ".format(
                time.time() - self.recordTime))
...
class swingManager():
    ...
    def __init__(self):
        ...
        # a/p swing status is switched in main loop async from crtl progam.
        # this var is used for GetDataHandler to get the correct swing no matter when handler is called
...
class scanManager():
    ...
    def __init__(self, restricted_frequency_list, channel):
        ...
        # 2nd period is triggered automatically before ROS finishes 1st.
        # if CLR_FRQ was requested for 1st => also do record on 2nd
        self.repeat_clrfreq_recording = False
        ...
        # is vert first trigger_next_period() call that just triggers first period  but does not collect cuda data
        self.isPrePeriod = True
    ...
    def wait_for_next_trigger(self):
        if self.syncBeams:
            time_to_wait = self.beam_times[self.current_period] - self.get_time_in_scan() \
                           - self.integration_time_manager.get_usrp_delay_time()
            if time_to_wait > 0:
                self.logger.debug("Waiting for {} s".format(time_to_wait))
                time.sleep(time_to_wait)
            else:
                self.logger.debug("No waiting. ({} + {}) s too late.".format(
                    time_to_wait + self.integration_time_manager.get_usrp_delay_time(),
                    self.integration_time_manager.get_usrp_delay_time()))

    def init_new_scan(self, freq_range_list, scan_beam_list, fixFreq, scan_times_list, scan_duration,
                      integration_duration, start_period):
    ...
    def get_nSec_to_scan_boundary(self, time_now):
        ...
        nSec_left = self.scan_duration - ((time.gmtime(time_now).tm_min * 60 + time.gmtime(
            time_now).tm_sec + time_now % 1) % self.scan_duration)
        return nSec_left
    ...
    def status(self):
        print("current period: {: >2d}/{}, beam: {} ".format(self.current_period, len(self.scan_beam_list),
                                                             self.current_beam))
    ...
    def get_current_clearFreq_result(self):
        if self.current_clrFreq_result is None:
            if self.fixFreq > 0:  # it looks like control program could use -1 and 0 to disable it
                ...
                self.logger.debug(
                    "  calc current clr_freq (ch {}, period {})".format(self.channel.cnum, self.current_period))
    ...
    def get_next_clearFreq_result(self):
        if self.next_clrFreq_result is None:
            if self.fixFreq != -1 and self.fixFreq != 0:  # control program could use -1 and 0 to disable it
                ...
                self.logger.debug(
                    "  calc next clr_freq (ch {}, period {})".format(self.channel.cnum, self.current_period + 1))
    ...
    def evaluate_clear_freq(self, iPeriod, beamNo):
        ...
        self.logger.debug(
            "clear_freq_range: {} on beam {} angle {}".format(self.clear_freq_range_list[iPeriod], beamNo, beam_angle))

        all_restricted_freq = self.restricted_frequency_list \
            + RHM.clearFreqRawDataManager.freq_occupied_by_other_channels
        self.logger.debug('start calc_clear_freq_on_raw_samples')
        clearFreq, noise = calc_clear_freq_on_raw_samples(rawData, metaData, all_restricted_freq,
                                                          self.clear_freq_range_list[iPeriod], beam_angle,
                                                          self.channel.raw_export_data['smsep'])
        ...
        self.logger.debug(
            "clear freq result for channel {}: selected {} , noise level {:2.1f}".format(self.channel.cnum, clearFreq,
                                                                                         noise))
    ...
    def isForelastPeriod(self):
        if self.camping:
            return False
        return self.current_period + 2 == len(self.scan_beam_list) or len(
            self.scan_beam_list) == 1  # for one beam scan: first is also forelast

    @property
    def isLastPeriod(self):
        if self.camping:
            return False
        return self.current_period + 1 == len(self.scan_beam_list) and len(
            self.scan_beam_list) != 1  # for one beam scan: first is not last
...
class RadarHardwareManager:
    def __init__(self, port):
        ...
        # waiting list for channels to be added at the right time (between two trigger_next() calls)
        self.newChannelList = []
    ...
    def run(self):
        ...
        def radar_main_control_loop():
            ...
            while True:
                ...
                if len(self.newChannelList) != 0:
                    self.logger.debug(
                        "active_channel list: {}".format([active_ch.cnum for active_ch in self.active_channels]))
                    ...
                    while (self.n_SetParameterHandlers_active):
                        self.logger.debug(
                            "Waiting for all {} SetParameterHandlers to finish before initializing new channels".format(
                                self.n_SetParameterHandlers_active))
                        time.sleep(0.001)
                ...
                if CS_TRIGGER in [ch.active_state for ch in self.channels]:
                    ...
                    for ch in self.channels:
                        if ch.active_state not in [CS_TRIGGER, CS_INACTIVE]:
                            controlLoop_logger.debug(
                                'remaining in TRIGGER because channel {} state is {} (active swing is {})'.format(
                                    ch.cnum, ch.active_state, ch.swingManager.activeSwing))
        ...
        while True:
            ...
            # TODO: the client_sock.accept is probably blocking,
            #  this will only execute after connection of a new control program. is that the intended behavior? -jtk
    ...
    def ini_file_init(self):
        ...
        self.hardwareLimit_freqRange = [float(array_config['hardware_limits']['minimum_tfreq']) / 1000, float(
            array_config['hardware_limits']['maximum_tfreq']) / 1000]  # converted to kHz
        ...
        while not usrps_synced:
            ...
            for iUSRP, usrpsock in enumerate(self.usrpManager.socks):
                try:
                    ...
                    self.logger.error("Error in sync USRPs for {}. Removing it...".format(
                        self.usrpManager.hostnameList_active[iUSRP]))
    ...
    def rxfe_init(self):
        ...
        if att < 0:
            self.logger.warning(
                'attenuation for rxfe in array.ini is defnined positive, '
                'but given value is negative ({} dB). correcting that to {} dB...'.format(
                    att, att * (-1)))
            att *= -1

        if att > 31.5:
            self.logger.warning(
                'attenuation ({}) for rxfe in array.ini is > 31.5 dB. using maximum atenuation of 31.5 dB'.format(att))
            att = 31.5
    ...
    def test_rxfe_control(self):
        ...
        testParSet = [[False, False, 0], [True, False, 0], [True, True, 0], [False, True, 0], [True, True, 31.5]] + [
            [False, False, 2 ** i / 2] for i in range(6)]
    ...
    def initialize_channel(RHM):
        """ Adds first period of channel for new channel or after CS_INACTIVE.
            Also appends channel to RHM.channels if not already done."""
        wait_start_time = time.time()
        while (time.time() - wait_start_time < 0.1) and (
                RHM.nControlPrograms > len(RHM.channels) + len(RHM.newChannelList)):
        ...
        # make a copy in case another channel is added during this function call
        newChannelList = RHM.newChannelList.copy()
        ...
        for channel in newChannelList:
            ...
            RHM.logger.debug(
                'calling CUDA_ADD_CHANNEL at initialize_channel() (cnum {}, swing {}, beam {})'.format(
                    channel.cnum, channel.swingManager.activeSwing, channel.scanManager.current_beam))
            cmd.transmit()
            cmd.client_return()
            if channel.active_state == CS_INACTIVE:
                RHM.logger.debug(
                    "initialize_channel() is setting ch {} swing {} from CS_INACTIVE to CS_READY".format(
                        channel.cnum, channel.swingManager.activeSwing))
                # channel not really ready until CUDA_GENERATE,
                # but there will be no trigger in parallel to this function
    ...
    def unregister_channel_from_HardwareManager(self, channelObject):
        ...
        if channelObject in self.channels:
            ...
            self.logger.info(
                'unregister_channel_from_HardwareManager() removing channel {} from HardwareManager'.format(
                    self.channels.index(channelObject)))
            ...
            for iSwing in range(nSwings):
                try:
                    ...
                    self.logger.error(
                        'unregister_channel_from_HardwareManager() failed to remove channel from HardwareManager')
    ...
    def _calc_period_details(self, newChannels=[]):
        ...
        self.logger.debug("_calc_period_details: samplingRate _bb={}, number_of_samples={}".format(
        ...
        #    nSamples_per_sequence = self.commonChannelParameter['number_of_samples'] + int(PULSE_SEQUENCE_PADDING_TIME
        ...
        self.logger.debug(
            "nSamples_per_sequence: {}, pulse_sequence_period: {}".format(nSamples_per_sequence, pulse_sequence_period))

        self.logger.debug("self.starttime_period: {}".format(self.starttime_period))
        self.logger.debug("self.commonChannelParameter['integration_period_duration: {}".format(
            self.commonChannelParameter['integration_period_duration']))

        # to find out how much time is available in an integration period for pulse sequences,subtract out startup delay
        # transmitting_time_left = self.starttime_period + self.commonChannelParameter['integration_period_duration']
        # - time.time() - self.integration_time_manager.get_usrp_delay_time()
        # - self.integration_time_manager.estimate_calc_time()

        time_now = time.time()
        nSec_to_end_of_period = self.commonChannelParameter[
                                    'integration_period_duration'] - time_now + self.starttime_period
        ...
        transmitting_time_left = nSec_to_end_of_period - self.integration_time_manager.estimate_calc_time() - \
                                 self.integration_time_manager.get_usrp_delay_time()
        ...
        if nSequences_per_period == 0:
            ...
            nSamples_per_sequence_if = int(downsamplingRates[1]) * (
                        (nSamples_per_sequence * nSequences_per_period) - 1) + int(
                downsamplingRates[1] * 2)  # assumes fixed nTaps for filter = 2*downsampling
            num_requested_rx_samples = int(downsamplingRates[0]) * (nSamples_per_sequence_if - 1) + int(
                downsamplingRates[0] * 2)  # assumes fixed nTaps for filter = 2*downsampling

        self.logger.debug(
            "RFIFRATE: {}, IFBBRATE: {}, nSamples_per_sequence_if: {}, nSamples_per_sequence: {}, "
            "nSequences_per_period: {}, NTapsRX_ifbb: {}, NTapsRX_rfif: {}".format(
                downsamplingRates[0], downsamplingRates[1], nSamples_per_sequence_if, nSamples_per_sequence,
                nSequences_per_period, downsamplingRates[0] * 2, downsamplingRates[1] * 2))

        self.logger.info("Effective integration time: {:0.3f}s = {} sequences ({}s) swing {}".format(
            num_requested_rx_samples / self.usrp_rf_tx_rate, nSequences_per_period,
            self.commonChannelParameter['integration_period_duration'], self.swingManager.activeSwing))
        ...
        pulse_sequence_offsets_samples = self.commonChannelParameter[
                                             'pulse_sequence_offsets_vector'] * self.usrp_rf_tx_rate

        # then, calculate sample indicies at which pulses start within an integration period
        # (all possible for cuda, only trasmitted for usrp_driver)
        # TODO if the pulse offsets change in one scan, this has to be changed
        #  (calc next pulse period for cuda, current for usrp driver)
        all_possible_integration_period_pulse_sample_offsets = np.zeros(
            nPulses_per_sequence * nSequences_per_period_max, dtype=np.uint64)
        for iSequence in range(nSequences_per_period_max):
            for iPulse in range(nPulses_per_sequence):
                all_possible_integration_period_pulse_sample_offsets[
                    iSequence * nPulses_per_sequence + iPulse] = np.round(
                    iSequence * self.nsamples_per_sequence + pulse_sequence_offsets_samples[iPulse])
        integration_period_pulse_sample_offsets = all_possible_integration_period_pulse_sample_offsets[
                                                  :nSequences_per_period * nPulses_per_sequence]
        ...
        for ch in self.channels + newChannels:
            ...
            assert abs(ch.nbb_rx_samples_per_sequence - pulse_sequence_period * self.commonChannelParameter[
                'baseband_samplerate']) < 1e4 / self.commonChannelParameter['baseband_samplerate'], \
                'pulse sequences lengths must be a multiple of the baseband sampling rate'
    ...
    def trigger_next_swing(self):
        ...
        nSamples_per_pulse = int(np.round(
        ...
        for ch in self.channels:
            self.logger.debug(
                'cnum {}: tfreq={}, rfreq={}, usrp_center={} rx_rate={}MHz '.format(
                    ch.cnum, ch.ctrlprm_struct.payload['tfreq'], ch.ctrlprm_struct.payload['rfreq'],
                    self.mixingFreqManager.current_mixing_freq, self.usrp_rf_tx_rate / 1e6))
        ...
        if transmittingChannelAvailable:
            if trigger_next_period:
                # USRP SETUP
                self.logger.debug('triggering period no {}, swing {}'.format(
                    channel.scanManager.current_period + 1 - channel.scanManager.isPrePeriod,
                ...
                if self.auto_collect_clrfrq_after_rx:
                    ...
                    auto_clear_freq_meta_data = dict(sampling_rate=self.usrp_rf_rx_rate,
                                                     center_freq=self.mixingFreqManager.current_mixing_freq * 1000)
                else:
                ...
                cmd = usrp_setup_command(self.usrpManager.socks, self.mixingFreqManager.current_mixing_freq * 1000,
                                         ...
                                         channel.integration_period_pulse_sample_offsets, self.swingManager.activeSwing)
                cmd.transmit()
                self.usrpManager.eval_client_return(cmd)
            ...
            if transmittingChannelAvailable and trigger_next_period and self.auto_collect_clrfrq_after_rx:
                auto_clear_freq_meta_data['record_time'] = usrp_integration_period_start_clock_time + (
                            nSamples_rx_requested_of_last_trigger / self.usrp_rf_rx_rate)
            ...
            for iSequence in range(n_sequence_times):
                pulse_start_time = usrp_integration_period_start_clock_time + iSequence * \
                                   self.nsamples_per_sequence / self.usrp_rf_rx_rate
            ...
            if trigger_next_period:
                ...
                cmd = usrp_trigger_pulse_command(self.usrpManager.socks, trigger_time,
                                                 self.commonChannelParameter['tr_to_pulse_delay'],
                                                 self.swingManager.activeSwing)
            ...
            for ch in self.channels:
                if ch.active_state == CS_TRIGGER:
                    ...
                    ch.logger.debug(
                        "Changing active channel state from CS_TRIGGER to CS_PROCESSING "
                        "(cnum: {}, swing {}, period {})".format(ch.cnum, self.swingManager.activeSwing,
                                                                 ch.scanManager.current_period))
            if trigger_next_period:
        ...
        if self.processing_swing_invalid:  # TODO check if this works (in control program or data files)
            ...
            if os.path.isfile("./bufferLiveData.flag"):
                ...
                chResExportList = [ch.resultDict_list[-1] for ch in self.channels if
                                   ch.processing_state == CS_PROCESSING]
                with open('tmpRawData.pkl', 'wb') as f:
                    pickle.dump([[], [], chResExportList, self.antenna_idx_list_main, self.antenna_idx_list_back], f,
            ...
            if CS_PROCESSING in allProcessingChannelStates:
                        ...
                        if channel.processing_state == CS_PROCESSING:
                            ...
                            for iAntenna in range(nAntennas):
                                ...
                                if main_samples is None:
                                    main_samples = np.zeros((len(self.channels), nMainAntennas, nSamples_bb),
                                                            dtype=np.complex64)
                                    back_samples = np.zeros((len(self.channels), nBackAntennas, nSamples_bb),
                                                            dtype=np.complex64)

                                samples = recv_dtype(cudasock, np.float32, nSamples_bb * 2)
                                # unpacked interleaved i/q
                                samples = (samples[0::2] + 1j * samples[
                                                                1::2]) * self.scaling_factor_rx_bb
                                ...
                                if antIdx in self.antenna_idx_list_main:
                                    ...
                                    self.logger.error(
                                        "Cuda tranmitted antenna ({}) that is not in main array list ({})"
                                        " and back array list ({}). (Maybe different antenna definitions "
                                        "in usrp_config.ini on both computers?)".format(
                                            antIdx, self.antenna_idx_list_main, self.antenna_idx_list_back))
                                    sys.exit(1)
                        else:
                            channel.logger.debug(
                                "Receiving NOTHING for channel {} because processing_state is {}".format(
                                    channel.cnum, channel.processing_state))
                ...
                beamformed_main_samples, beamformed_back_samples = self.calc_beamforming(main_samples, back_samples,
                                                                                         antenna_scale_factors)
                for iChannel, channel in enumerate(self.channels):
                    if channel.processing_state == CS_PROCESSING:
                        # copy samples and ctrlprm to transmit later to control program
                        if 'main_beamformed' in channel.resultDict_list[-1]:
                            channel.logger.error("Main beamformed data already exist. Overwriting it. "
                                                 "This is not correct. GetDataHandler too slow??")
                        ...
                        channel.update_ctrlprm_class("current")
                        channel.resultDict_list[-1]['ctrlprm_dataqueue'] = copy.deepcopy(
                            channel.ctrlprm_struct.dataqueue)
                        ...
                        if os.path.isfile("./save.raw.bb"):
                            ...
                            channel.bb_export['number_of_sequences'] = channel.resultDict_list[-1][
                                'npulses_per_sequence']
                            ...
                            channel.bb_export['nSequences_per_period'] = channel.resultDict_list[-1][
                                'nSequences_per_period']
                            channel.bb_export['sequence_start_time_secs'] = channel.resultDict_list[-1][
                                'sequence_start_time_secs']
                            channel.bb_export['sequence_start_time_usecs'] = channel.resultDict_list[-1][
                                'sequence_start_time_usecs']
                            channel.bb_export['nbb_rx_samples_per_sequence'] = channel.resultDict_list[-1][
                                'nbb_rx_samples_per_sequence']
                ...
                if os.path.isfile("./bufferLiveData.flag"):
                    ...
                    chResExportList = [ch.resultDict_list[-1] for ch in self.channels if
                                       ch.processing_state == CS_PROCESSING]
                    with open('tmpRawData.pkl', 'wb') as f:
                        pickle.dump([main_samples, back_samples, chResExportList, self.antenna_idx_list_main,
                                     self.antenna_idx_list_back], f, pickle.HIGHEST_PROTOCOL)
                    os.rename("tmpRawData.pkl", "liveRawData.pkl")
                    os.remove("./bufferLiveData.flag")

                # save IF raw data
                for channel in self.channels:
                    if channel.processing_state == CS_PROCESSING and os.path.isfile(
                            "/collect.if.{:c}".format(97 + channel.cnum)):
        ...
        for iChannel, channel in enumerate(self.channels):
            if channel.processing_state is CS_PROCESSING:
                ...
                if channel.scanManager.isPostLast:  # or channel.scanManager.isForelastPeriod:
                    ...
                    # self.nRegisteredChannels -= 1
                    channel.logger.debug(
                        'last period finished, setting active and next processing state to CS_INACTIVE')
                elif channel.scanManager.isLastPeriod:
                    channel.next_processing_state = CS_LAST_SWING
                else:
                    channel.next_processing_state = CS_READY
                channel.logger.debug("Switching next processing state (swing {}) of cnum {} to {}".format(
                    self.swingManager.processingSwing, channel.cnum, channel.next_processing_state))

                channel.logger.debug(
                    "Switching processing state (swing {}) state of cnum {} "
                    "from CS_PROCESSING to CS_SAMPLES_READY".format(self.swingManager.processingSwing, channel.cnum))
        ...
        for channel in self.channels:
            if channel.scanManager.isLastPeriod:  # or channel.scanManager.isForelastPeriod:
                ...
                self.logger.debug('send CUDA_REMOVE_CHANNEL (cnum {}, swing {})'.format(
                    channel.cnum, self.swingManager.processingSwing))
                cmd.transmit()
                cmd.client_return()
                ...
                if channel.active:
                    ...
                    cmd = cuda_add_channel_command(self.cuda_sockets, sequence=channel.get_next_sequence(),
                                                   swing=self.swingManager.processingSwing)
                    self.logger.debug('send CUDA_ADD_CHANNEL (cnum {}, swing {}, beam {})'.format(
                        channel.cnum, self.swingManager.processingSwing, channel.scanManager.current_beam))
                    cmd.transmit()
                    cmd.client_return()
                    ...
                    if channel.processing_state == CS_INACTIVE:  # first use of swing 1
                        ...
                        channel.logger.debug(
                            "Switching processing state (swing {}) state of cnum {} to CS_READY "
                            "(first use of swing)".format(self.swingManager.processingSwing, channel.cnum))
                else:
        ...
        if transmittingChannelAvailable and trigger_next_period:
            ...
            for iUSRP, ready_return in enumerate(payloadList):
                if ready_return == CONNECTION_ERROR:
                    ...
                    if rx_status < 0:
                        rx_error_codes = dict(ERROR_CODE_NONE=0x0, ERROR_CODE_TIMEOUT=0x1, ERROR_CODE_LATE_COMMAND=0x2,
                                              ERROR_CODE_BROKEN_CHAIN=0x4, ERROR_CODE_OVERFLOW=0x8,
                                              ERROR_CODE_ALIGNMENT=0xc, ERROR_CODE_BAD_PACKET=0xf,
                                              WRONG_NUMBER_OF_SAMPLES=100)
                        ...
                        self.logger.error(
                            "Error: {}  (code {}) occurred in rx_worker for antennas {}.".format(
                                print_name, rx_status,
                                self.usrpManager.antennaList_active[iUSRP - antenna_list_offset]))
        ...
        self.logger.debug('switching swings to: active={}, processing={}'.format(self.swingManager.activeSwing,
        ...
        if transmittingChannelAvailable:
            ...
            for channel in self.channels:
                if channel.scanManager.isFirstPeriod:
                    channel.logger.debug(
                        'setting active state (cnum {}, swing {}) to CS_TRIGGER to start second period'.format(
                            channel.cnum, self.swingManager.activeSwing))
        ...
        if transmittingChannelAvailable and trigger_next_period and self.auto_collect_clrfrq_after_rx:
            ...
            self.clearFreqRawDataManager.update_auto_clear_freq_data(antenna_list, clr_samples,
                                                                     auto_clear_freq_meta_data)
            cmd.client_return()
    ...
    def no_channel_gain_control(self, nChannelsWillBeAdded=0):
        self.logger.debug("No channel scaling. Global factor for all channels is: totalScaligFactor  = {} ".format(
            self.scaling_factor_tx_total))
    ...
    def gain_control_divide_by_nChannels(self, nChannelsWillBeAdded=0):
        ...
        self.logger.debug("Setting channel scaling factor to: totalScaligFactor / nChannels = {}/ {} ".format(
            self.scaling_factor_tx_total, nChannels))
        for ch in self.channels + self.newChannelList:
    ...
    def calc_normalize_and_mute_factors(RHM, main_samples, back_samples):
        ...
        if RHM.apply_normalization:
            ...
            for iChannel, channel in enumerate(RHM.channels):
                ...
                RHM.logger.info(
                    "max var = {:2.3f} = {:2.3f} **2, var_threshold = {} (-30 dB) ".format(max_var, np.sqrt(max_var),
                                                                                           var_threshold))
                for iAntenna in range(nAntennas_main):
                    if antenna_scale_factors[iChannel][RHM.antenna_idx_list_main[iAntenna]]:
                        if var_list[iAntenna] > var_threshold:
                            scale_factor = np.sqrt(max_var / var_list[iAntenna]) * antenna_scale_factors[iChannel][
                                RHM.antenna_idx_list_main[iAntenna]]
                            antenna_scale_factors[iChannel][RHM.antenna_idx_list_main[iAntenna]] = scale_factor
                            RHM.logger.info(
                                "scaling antenna {} with factor {:}".format(RHM.antenna_idx_list_main[iAntenna],
                                                                            scale_factor))
                        else:
                            RHM.logger.info("not scaling antenna {} because of small variance: {} (< threshold)".format(
                                RHM.antenna_idx_list_main[iAntenna], var_list[iAntenna]))
                            scale_factor = 1
                    else:
                        RHM.logger.info("muting antenna {} (defined in array_config.ini)".format(
                            RHM.antenna_idx_list_main[iAntenna], var_list[iAntenna]))
                ...
                for iAntenna in range(nAntennas_back):
                    if antenna_scale_factors[iChannel][RHM.antenna_idx_list_back[iAntenna]]:
                        if var_list[iAntenna + nAntennas_main] > var_threshold:
                            scale_factor = np.sqrt(max_var / var_list[iAntenna + nAntennas_main]) * \
                                           antenna_scale_factors[iChannel][RHM.antenna_idx_list_back[iAntenna]]

                            antenna_scale_factors[iChannel][RHM.antenna_idx_list_back[iAntenna]] = scale_factor
                            RHM.logger.info(
                                "scaling antenna {} with factor {:}".format(RHM.antenna_idx_list_back[iAntenna],
                                                                            scale_factor))
                        else:
                            RHM.logger.info("not scaling antenna {} because of small variance: {} (< threshold)".format(
                                RHM.antenna_idx_list_back[iAntenna], var_list[iAntenna + nAntennas_main]))
                            scale_factor = 1
                    else:
                        RHM.logger.info("muting antenna {} (defined in array_config.ini)".format(
                            RHM.antenna_idx_list_back[iAntenna], var_list[iAntenna + nAntennas_main]))
    ...
    def calc_beamforming(RHM, main_samples, back_samples, antenna_scale_factors):
        ...
        for iChannel, channel in enumerate(RHM.channels):
            if channel.processing_state is CS_PROCESSING:
                bmazm = calc_beam_azm_rad(RHM.array_nBeams, channel.scanManager.current_beam,
                                          RHM.array_beam_sep)  # calculate beam azimuth from transmit beam number
                channel.logger.debug(
                    "rx beamforming: ch {}, beam {}".format(channel.cnum, channel.scanManager.current_beam))
                ...
                pshift = calc_phase_increment(bmazm, clrFreqResult[0] * 1000., RHM.array_x_spacing)
                ...
                phasing_matrix = np.matrix(
                    [rad_to_rect(ant_idx * pshift) * antenna_scale_factors[iChannel][ant_idx] for ant_idx in
                     first_pol_ant_idx])  # calculate a complex number representing the phase shift for each antenna
                ...
                RHM.logger.info(
                    "Abs max_value is {} (int16_max= {}, "
                    "max_value / int16_max = {} ) ".format(abs_max_value, maxInt16value, abs_max_value / maxInt16value))

                # check for clipping
                if (real_mat > maxInt16value).any() or (real_mat < minInt16value).any() or (
                        imag_mat > maxInt16value).any() or (imag_mat < minInt16value).any():
                ...
                phasing_matrix = np.matrix(
                    [rad_to_rect((ant_idx - 10) * pshift) * antenna_scale_factors[iChannel][ant_idx] for ant_idx in
                     RHM.antenna_idx_list_back])
...
class RadarChannelHandler:
    def __init__(self, conn, parent_RadarHardwareManager):
        ...
        self.logger = logging.getLogger(
            "ChManager #{}".format(parent_RadarHardwareManager.channel_manager_consecutive_number))
        ...
        # reference to global swingManager of RadarHardwareManager
        self.swingManager = parent_RadarHardwareManager.swingManager
    ...
    def run(self):
        ...
        while True:
            ...
            command = chr(
                rmsg.payload['type'] & 0xFF)  # for some reason, libtst is sending out 4 byte commands with junk..
            try:
                self.logger.debug('ch {}: received command (ROS=>USRP_Server): {}, {}'.format(self.cnum, command,
                                                                                              RMSG_COMMAND_NAMES[
                                                                                                  command]))
            except KeyError:
            ...
            try:
                ...
                self.logger.error('ch {}: Error while command {} ({}). Removing this channel'.format(
                    self.cnum, RMSG_COMMAND_NAMES[command], command))
    ...
    def _waitForState(self, swing, state):
        ...
        while self.state[swing] not in state:
            ...
            if counter == 1:
                self.logger.debug(
                    "ch {}:_waitForState {}. state is {} (swing {})".format(self.cnum, state, self.state[swing], swing))
    ...
    def get_current_sequence(self, remove_channel=False):
        if not remove_channel:
            ...
            self.logger.debug("Getting current sequence with {} samples (305x1500x {}) rbeam {}".format(
                self.nrf_rx_samples_per_integration_period, self.nrf_rx_samples_per_integration_period / 305 / 1500,
                self.ctrlprm_struct.payload['rbeam']))

        seq = sequence(self.npulses_per_sequence, self.tr_to_pulse_delay,
                       self.parent_RadarHardwareManager.all_possible_integration_period_pulse_sample_offsets,
                       self.pulse_lens, self.phase_masks, self.pulse_masks, self.channelScalingFactor,
                       self.ctrlprm_struct.payload)
    ...
    def RequestAssignedFreqHandler(self, rmsg):
        ...
        self.logger.info(
            'ch {}: clr frequency search raw data age: {:2.1f} s'.format(self.cnum, time.time() - clrFreqResult[2]))
        return RMSG_SUCCESS
    ...
    def RequestClearFreqSearchHandler(self, rmsg):
        ...
        if self.scanManager.fixFreq <= 0:
            ...
            self.logger.debug(
                "RequestClearFreqSearchHandler: "
                "setting request CLR_FREQ flag in clearFreqRawDataManager (caused by ch {})".format(
                    self.cnum))
        else:
    ...
    def SetReadyFlagHandler(self, rmsg):
        ...
        self.logger.debug(
            "ch {}: SetReadyFlagHandler: "
            "waiting for nextSwingToTrigger (swing {}) become CS_READY or CS_LAST_SWING".format(
                self.cnum, self.swingManager.nextSwingToTrigger))
        ...
        self.logger.debug(
            "ch {}: SetReadyFlagHandler: setting nextSwingToTrigger state (swing {}) to CS_TRIGGER".format(
                self.cnum, self.swingManager.nextSwingToTrigger))
    ...
    def RegisterSeqHandler(self, rmsg):
        ...
        self.logger.debug(
            "pulse0 length: {} us, tr_pulse_delay: {} us, tx_time: {} us".format(self.pulse_lens[0], tr_to_pulse_delay,
                                                                                 self.pulse_lens[
                                                                                     0] + 2 * self.tr_to_pulse_delay))
    ...
    def write_bb_data(channel):
        ...
        fileName = '{:04d}{:02d}{:02d}{:02d}{:02d}.{:d}.iraw.{:c}'.format(time_now.year, time_now.month, time_now.day,
                                                                          time_now.hour, time_now.minute, channel.rnum,
                                                                          96 + channel.cnum)
        ...
        for iSequence in range(channel.bb_export['nSequences_per_period']):
            ...
            for iAntenna in range(channel.bb_export['nMainAntennas']):
                export_samples += float_main_data[iAntenna][
                                  pulse_sequence_start_index:pulse_sequence_end_index].tobytes()
            for iAntenna in range(channel.bb_export['nBackAntennas']):
                export_samples += float_back_data[iAntenna][
                                  pulse_sequence_start_index:pulse_sequence_end_index].tobytes()
    ...
    def SetParametersHandler(self, rmsg):
        ...
        if self.scanManager.isInitSetParameter:
            ...
            self.logger.debug(
                "ch {}: Received from ROS for swing {} (init SetPar is only stored): "
                "tbeam={}, rbeam={}, tfreq={}, rfreq={}".format(
                    self.cnum, current_swing, self.ctrlprm_struct.payload['tbeam'],
                    self.ctrlprm_struct.payload['rbeam'], self.ctrlprm_struct.payload['tfreq'],
                    self.ctrlprm_struct.payload['rfreq']))
            RHM.n_SetParameterHandlers_active -= 1
            return RMSG_SUCCESS

        self.logger.debug(
            "ch {}: Received from ROS SetParameter for swing {} : tbeam={}, rbeam={}, tfreq={}, rfreq={}".format(
                self.cnum, current_swing, self.ctrlprm_struct.payload['tbeam'], self.ctrlprm_struct.payload['rbeam'],
                self.ctrlprm_struct.payload['tfreq'], self.ctrlprm_struct.payload['rfreq']))
        ...
        if self.state[
            current_swing] == CS_INACTIVE:  # or self.active_state == CS_READY:#  not needed with change of site.c
            ...
            self.ctrlprm_struct.receive(self.conn)
            self.logger.debug("ch {}: Received from ROS: tbeam={}, rbeam={}, tfreq={}, rfreq={}".format(
                self.cnum, self.ctrlprm_struct.payload['tbeam'], self.ctrlprm_struct.payload['rbeam'],
                self.ctrlprm_struct.payload['tfreq'], self.ctrlprm_struct.payload['rfreq']))
            ...
            for key in ctrlprm_old.keys():
                if np.any(ctrlprm_old[key] != self.ctrlprm_struct.payload[key]):
                    if key == "tfreq" and self.ctrlprm_struct.payload[
                        key] == 12000:  # control program always sends 2 SET_PAR. 1st one with tfreq 12MHz
                        continue

                    self.logger.error(
                        "ch {}: received ctrlprm_struct for {} ({}) is not equal with prediction ({})".format(
                            self.cnum, key, self.ctrlprm_struct.payload[key], ctrlprm_old[key]))
                    # TODO return RMSG_FAILURE
                # else:
                #  self.logger.debug("ch {}: received ctrlprm_struct for {} ({})
                #  IS equal with prediction ({})".format(self.cnum,
                #  key,self.ctrlprm_struct.payload[key], ctrlprm_old[key] ))
        else:
            self.logger.error(
                "ROS:SetParameter: Active state is {} (current_swing={}, activeSwing={} ). "
                "Dont know what to do...".format(
                    self.state[current_swing], self.swingManager.activeSwing, self.active_state))
        ...
        if (self.rnum < 0 or self.cnum < 0):
            self.logger.error(
                "SET_PARAMETER: Invalid radar or channel number: rnum={}, cnum={}".format(self.rnum, self.cnum))
            RHM.n_SetParameterHandlers_active -= 1
            return RMSG_FAILURE
    ...
    def CheckChannelCompatibility(self):
        ...
        commonParList_seq = ['npulses_per_sequence', 'pulse_sequence_offsets_vector', 'tr_to_pulse_delay',
                             'integration_period_duration', 'tx_time']
        if all([self.pulse_lens[0] == self.pulse_lens[i] for i in range(1, len(self.pulse_lens))]):
            ...
            pdb.set_trace()
            return False
        if hardwareManager.nRegisteredChannels == 1 and (
                len(hardwareManager.channels) == 0 or hardwareManager.channels[0] == self):
            self.logger.info(
                "Compatibility check: This channel is already registered at HardwareManager and is the only one. "
        ...
        if hardwareManager.nRegisteredChannels == 0:  # this is the first channel
            ...
            hardwareManager.commonChannelParameter.update(
                {key: self.ctrlprm_struct.payload[key] for key in commonParList_ctrl})
            ...
            total_downsample_rate = hardwareManager.usrp_rf_rx_rate / hardwareManager.commonChannelParameter[
                'baseband_samplerate']
            ...
            if downSampleRates is None:
                errorMsg = "No downsample rates are defined for downsampling from {} to {}.".format(
                    hardwareManager.usrp_rf_rx_rate, hardwareManager.commonChannelParameter['baseband_samplerate'])
                self.logger.error(errorMsg)
                assert downSampleRates != None, errorMsg
            else:
                self.logger.debug(
                    "Setting cuda downsampling ratios to {} and {}".format(downSampleRates[0], downSampleRates[1]))
            ...
            parCompatibleList_seq = [hardwareManager.commonChannelParameter[parameter] == getattr(self, parameter) for
                                     parameter in commonParList_seq]
            parCompatibleList_ctrl = [
                hardwareManager.commonChannelParameter[parameter] == self.ctrlprm_struct.payload[parameter] for
                parameter in commonParList_ctrl]
            ...
            #   pdb.set_trace()
            if (not all(parCompatibleList_seq)) or (not all(parCompatibleList_ctrl)) or (
                    pulseLength != hardwareManager.commonChannelParameter['pulseLength']):
                ...
                for iPar, isCompatible in enumerate(parCompatibleList_seq):
                    if not all(isCompatible):
                        self.logger.error(
                            " Not compatible sequence parameter: {}   old channel(s): {} , new channel: {}".format(
                                commonParameterList_seq[iPar],
                                hardwareManager.commonChannelParameter[commonParameterList_seq[iPar]],
                                getattr(self, commonParameterList_seq[iPar])))
                for iPar, isCompatible in enumerate(parCompatibleList_ctrl):
                    if not isCompatible:
                        self.logger.error(" Not compatible ctrlprm: {}   old channel(s): {} , new channel: {}".format(
                            commonParameterList_ctrl[iPar],
                            hardwareManager.commonChannelParameter[commonParameterList_ctrl[iPar]],
                            self.ctrlprm_struct.payload[commonParameterList_ctrl[iPar]]))
                if pulseLength != hardwareManager.commonChannelParameter['pulseLength']:
                    self.logger.error(
                        " Pulse length of new channel ({}) is not compatible to old channel(s) ({})".format(
                            pulseLength, hardwareManager.commonChannelParameter['pulseLength']))
                return False
            else:
        ...
        assert np.abs((ch.ctrlprm_struct.payload['tfreq'] * 1e3) - self.mixingFreqManager.current_mixing_freq * 1e6) < (
                    self.usrp_rf_tx_rate / 2), \
            'transmit frequency outside range supported by sampling rate and center frequency'
    ...
    def GetParametersHandler(self, rmsg):
        ...
        self.logger.debug(
            "ch {}: sending current ctrlprm_struct (tfreq={}, rfreq={},tbeam={},rbeam={})".format(
                self.cnum, self.ctrlprm_struct.get_data('tfreq'), self.ctrlprm_struct.get_data('rfreq'),
                self.ctrlprm_struct.get_data('tbeam'), self.ctrlprm_struct.get_data('rbeam')))
        return RMSG_SUCCESS
    ...
    def GetDataHandler(self, rmsg):
        ...
        # only 'samples' of dataprm is ever changed TODO check other parameter such as event_secs....
        ...
        self.logger.debug(
            'ch {}: channelHanlder:GetDataHandler waiting for channel to idle before GET_DATA '
            '(finished swing is {})'.format(
                self.cnum, finishedSwing))
        ...
        self.logger.debug(
            'ch {}: channelHanlder:GetDataHandler finished returning samples. setting state to {}  (swing {})'.format(
                self.cnum, self.next_state[finishedSwing], finishedSwing))
    ...
    def send_results_to_control_program(self):
        ...
        # TODO is this the right way to return fault status????
        txstatus_agc = self.parent_RadarHardwareManager.usrpManager.fault_status
        txstatus_lowpwr = np.zeros(num_transmitters)
        if txstatus_agc.any():
            self.logger.warning('Following USRPs report Fault:  {} (usrp index)'.format(
                [k for k in range(txstatus_agc.size) if txstatus_agc[k] != 0]))
        ...
        for iSequence in range(resultDict['nSequences_per_period']):
            ...
            # self.logger.debug("Number of samples if {} (no deepcopy version is {}), main beamformed shape:
            # {}".format(resultDict['number_of_samples'], rd_shallow['number_of_samples'],
            # resultDict['main_beamformed'].shape))
            # self.logger.debug("start index: {}, end index: {}".format(pulse_sequence_start_index,
            # pulse_sequence_end_index))

            # send the packed complex int16 samples to the control program.. 
            # self.logger.debug('GET_DATA sending main samples')
            transmit_dtype(self.conn,
                           resultDict['main_beamformed'][pulse_sequence_start_index:pulse_sequence_end_index],
                           np.uint32)

            # self.logger.debug('GET_DATA sending back samples')
            transmit_dtype(self.conn,
                           resultDict['back_beamformed'][pulse_sequence_start_index:pulse_sequence_end_index],
                           np.uint32)
    ...
    def SetRadarChanHandler(self, rmsg):
        ...
        if self.cnum in [ch.cnum for ch in self.parent_RadarHardwareManager.channels if
                         ch is not None and ch is not self]:
            self.logger.error(
                "New channel (cnum {}) can not be added beause channel with this cnum already active.".format(
                    self.cnum))
            return RMSG_FAILURE
    ...
    def QueryIniSettingsHandler(self, rmsg):
        ...
        ini_name = recv_dtype(self.conn, str, number_of_items=data_length)
    ...
    def SetActiveHandler(self, rmsg):
        ...
        clrfreq_start_list = recv_dtype(self.conn, np.int32, number_of_items=scan_num_beams)
        clrfreq_bandwidth_list = recv_dtype(self.conn, np.int32, number_of_items=scan_num_beams)
        scan_beam_list = recv_dtype(self.conn, np.int32, number_of_items=scan_num_beams)
        ...
        if syncBeams == 1:
            scan_times_list = recv_dtype(self.conn, np.int32, number_of_items=scan_num_beams) / 1000
        ...
        if self.scanManager.camping:
            ...
            self.logger.info(
                "SetActiveHandler: Skipping reset of scanManager and swingManager "
                "because nothing changed with camping beam.")
            return RMSG_SUCCESS
        ...
        freq_range_list = [[clrfreq_start_list[i], clrfreq_start_list[i] + clrfreq_bandwidth_list[i]] for i in
                           range(scan_num_beams)]

        self.logger.debug('SetActiveHandler updating swingManager with new freq/beam lists')
        self.scanManager.init_new_scan(freq_range_list, scan_beam_list, fixFreq, scan_times_list, scan_time,
                                       integration_time, start_period)
        self.triggered_swing_list = []
        ...
        if addFreqResult == True:
            ...
            # self.logger.debug("Resetting swing manager (active={}, processing={})".format(
            # self.swingManager.activeSwing, self.swingManager.processingSwing ))
            return RMSG_SUCCESS
        elif addFreqResult == False:
            self.logger.error(
                "Freq range of new channel (cnum {}) is not in USRP bandwidth. (freq_range_list[0][0] = {} ".format(
                    self.cnum, freq_range_list[0][0]))
            ...
            self.parent_RadarHardwareManager.clearFreqRawDataManager.center_freq = \
                self.parent_RadarHardwareManager.mixingFreqManager.current_mixing_freq
            self.parent_RadarHardwareManager.clearFreqRawDataManager.metaData[
                'usrp_fcenter'] = self.parent_RadarHardwareManager.mixingFreqManager.current_mixing_freq
            # self.swingManager.reset()
            # self.logger.debug("Resetting swing manager (active={}, processing={})".format(
            # self.swingManager.activeSwing, self.swingManager.processingSwing ))
            return RMSG_SUCCESS
    ...
    def SetInactiveHandler(channelObject, rmsg):
        ...
        if channelObject in RHM.active_channels:
            RHM.logger.info('ROS:SET_INACTVIVE removing channel {} from RHM.active_channels'.format(
                RHM.channels.index(channelObject)))
            RHM.active_channels.remove(channelObject)

        if channelObject in RHM.channels:
            RHM.logger.info(
                'ROS:SET_INACTVIVE removing channel {} from HardwareManager'.format(RHM.channels.index(channelObject)))
